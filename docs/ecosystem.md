...
> Wiki 文档版本：`v1.0.2` · 更新日期：`2026-09-23`（项目关系独立版本）
...
`dsh-windows-tool-fix` 关注“命令如何在 Windows 上执行”：它通过 DSH profile patch 把 `minimal-gitbash` 设为默认 Agent preset，绕开 Windows 不支持的持久 PTY 检查。它是三个 DSH 插件中唯一处理执行层的，不解析会话也不读取模型能力。
...
`Sakura-MCP-Server` 当前为 `v0.3.4`，已落地客户端会话观测、登录 Cookie 修复和登录页系统字体改造；`Life Dashboard` 当前为 `1.0.11`，已落地静默 SSO、续期诊断、登录页视觉重做与加载页品牌动画；`Local Model Gateway` 当前为 `v2.2.0`，提供 Windows 桌面客户端、用量导出、每上游可配置重试和统一错误码；`Sakura-AiCut` 当前为 `0.2.1`，`UniLink` 当前为 `v1.2`，`Sakura-Chat` 暂无 Release tag。各项目可独立部署，组合使用时不互相耦合。
