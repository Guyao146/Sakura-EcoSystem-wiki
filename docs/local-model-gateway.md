...
`v2.2.0` 新增**每个上游重试次数**：某个上游出现连接失败、超时、429 或 5xx 时，先原地重试同一个上游这么多次，仍失败才切换到下一优先级的备用上游；4xx 参数/鉴权类错误不重试也不切换，直接返回，避免掩盖配置问题。

错误消息前缀的格式在 `v2.1.0` 后统一为 `自定义前缀 [request_id=请求ID] [code=错误码] 原始报错内容`，例如 `[SakuraGateway] [request_id=req_xxx] [code=400] model 不能为空`。错误码优先使用上游 `code`，缺失时使用 HTTP 状态码；流内错误没有 HTTP 状态码时用 `502`。留空只省略自定义前缀，请求 ID 和错误码仍显示。
...
| 版本 | 要点 |
| --- | --- |
| `v2.0.10` | 修复 Responses 转 Chat 流式响应中工具调用 ID 关联错乱：工具输出项 ID、调用 ID 和输出序号关联到同一个 Chat 工具 index，保证工具首帧携带字符串 ID |
| `v2.1.0` | 统一 JSON 与 SSE 错误中的请求 ID 和错误码；Chat 模式下 function tools 与 `reasoning_effort` 可同时传递，不再因为开启思考而切换协议 |
| `v2.1.1` | 请求日志记录结构化的上游错误信息，便于排查 |
| `v2.1.2` | 修复管理后台中文乱码（`v2.1.1` 引入的回归） |
