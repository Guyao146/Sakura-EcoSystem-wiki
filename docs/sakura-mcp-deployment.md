...
> Wiki 文档版本：`v1.0.1` · 更新日期：`2026-09-20`（Sakura-MCP-Server 生产部署独立版本，固定 tag 升级到 `v0.3.4`）

> [!WARNING]
> `v0.3.4` 已创建正式 Release。生产部署建议固定 `v0.3.4` tag，先在测试环境完成安装、备份恢复、权限和模型 Provider 演练。
...
生产环境使用 `v0.3.4` 或经过 CI 验证的 commit，不要长期无审查跟随 `main`。
...
curl -fsSLO https://raw.githubusercontent.com/Guyao146/Sakura-MCP-Server/v0.3.4/.env.example
cp .env.example .env
```
...
`v0.3.4` 的生产 Compose 可以在空目录中只放一个 `docker-compose.yml`，然后直接启动：
