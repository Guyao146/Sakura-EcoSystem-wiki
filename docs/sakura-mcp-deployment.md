...
生产环境使用 `v0.3.3` 或经过 CI 验证的 commit，不要长期无审查跟随 `main`。
...
curl -fsSLO https://raw.githubusercontent.com/Guyao146/Sakura-MCP-Server/v0.3.3/.env.example
cp .env.example .env
```
...
```dotenv
SAKURA_MCP_IMAGE=ghcr.io/guyao146/sakura-mcp-server:0.3.3
```
...
`v0.3.3` 的生产 Compose 可以在空目录中只放一个 `docker-compose.yml`，然后直接启动：
