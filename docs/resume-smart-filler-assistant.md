...
3. 点击 **加载已解压的扩展程序**，选择包含 `manifest.json`、`popup/`、`background/`、`content/` 的仓库根目录。
4. 点击工具栏中的扩展图标，在 **设置** 里选择 OpenAI / Moonshot / 自定义接口，填写 API Key 并保存。
5. 上传 PDF / Word / TXT 简历（单个文件最大 `10 MB`），或点击 **手动粘贴简历文本**（至少 20 个字符）。
6. 打开招聘表单页面，点击 **扫描并填充当前页面**；查看识别字段、成功填充和未匹配统计。
7. 未匹配的字段用 **手动补充填写** 逐项处理（快捷键 `Ctrl + Shift + R` 打开或关闭面板）。

> 仓库没有 `package.json` 和构建步骤，直接加载源码目录即可。扩展**不会自动提交表单**，填入的内容需要人工确认后再提交。AI 匹配失败时不会猜测值覆盖字段，可放心重试。
